...
const PUBLIC_PATHS = {
  ...
  BUY: {
    ...
    PRODUCT_ONE: (productId = ROUTE_PARAMS.productId) => `${productId}`,
  },
  // put user_shop below because need specific routes first
  USER_SHOP: (userUsername = ROUTE_PARAMS.userUsername) => `${userUsername}`,
  SERVER: `server-error`,
  ERROR: '**',
};

const USER_PATHS = {
  USER: (userUsername = ROUTE_PARAMS.userUsername) => `${userUsername}`,
  ABOUT: {
  ...
  GROUP: {
    ROOT: `groups`,
    CREATE_GROUP: `create`,
    EDIT_GROUP: (groupId = ROUTE_PARAMS.groupId) => `${groupId}`,
  },
  BUYER_CART: {
    ROOT: `cart`,
    CHECKOUT: `checkout`,
  ...
  BUYER_ORDERS: {
    ROOT: `orders`, // orders
    ORDER_ONE: (orderId = ROUTE_PARAMS.orderId) => `${orderId}`,
  },
  SELLER: {
    ROOT: `sell`,
    PRODUCTS: {
...
const URLS = {
  ...
  USER: (userUsername = ROUTE_PARAMS.userUsername) => {
